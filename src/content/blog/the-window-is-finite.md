...
**Fact position.** Hide one sentence (`The inventory code for the spare projector is PX-441.`) at the start, the middle, or the end of a padded facilities handbook. Ask the same question: what is the inventory code? Record which positions return `PX-441`. That's the check I'd actually run on a handbook paste, because the projector code is the kind of boring fact that lives in the middle of a PDF.

Lost-in-the-middle is a published failure mode: models often use the beginning and the end of a long context more reliably than the middle. A given run may or may not show it. Write that down. Position in the window isn't free either way.
...
Count how often `PX-441` comes back. If middle is worse than start and end, you saw lost-in-the-middle on this model and this padding. If it isn't, write that down too. A single lucky middle hit isn't a design.
...
Short `max_tokens` on a classification is fine. Short `max_tokens` on "explain this policy" looks like a product that can't finish a sentence. Pin the limit next to the prompt in the eval log, the same way you pin temperature.
